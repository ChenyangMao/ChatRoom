        ...
        try {
          ...
          try {
            while (dis.available() > 0) {
              ...
              File file = new File(
                  "C:\\Users\\cheny\\IdeaProjects\\chatting\\chatting-client\\src\\main\\java\\cn\\edu\\sustech\\cs209\\chatting\\client\\"
                      + name + "_receive_" + fileName);
  ...
  public void send(Message message) {
    if (message.getChatName() != null) {
      if (!messages.containsKey(message.getChatName())) {
        ...
        boolean flag = true;
        for (Message m1 : m) {
          if (m1.getTimestamp().equals(message.getTimestamp()) && m1.getSentBy()
              .equals(message.getSentBy())) {
            flag = false;
          }
        }
        if (flag) {
